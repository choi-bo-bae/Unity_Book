...
public class GameManager : MonoBehaviour
{
    ...
    void InventorySetup()
    {
        ...
        for(int i = 0; i < gameData.equipment.Count; i++)
        {
            for(int j = 0; j < slots.Length; j++)
            {
                ...
                int itemIndex = (int)gameData.equipment[i].itemType;

                itemObjects[itemIndex].GetComponent<Transform>().SetParent(slots[j]);

                itemObjects[itemIndex].GetComponent<ItemInfo>().itemData = gameData.equipment[i];
    ...
    {
        if (gameData.equipment.Contains(item)) return;

        gameData.equipment.Add(item);
    ...
    {
        gameData.equipment.Remove(item);
